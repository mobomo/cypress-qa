...
function buildStepNav(items, itemHeading, itemsSeen, linktoFn) {
  var nav = '';
  var itemsNav = '';

  if (items && items.length) {
    items.forEach(function (item) {
      var steps = find({kind: 'step', group: item.group});

      if (!hasOwnProp.call(item, 'group')) {
        itemsNav += '<li id="' + item.group.replace('/', '_') + '-nav">' + linktoFn('', item.group);
        itemsNav += '</li>';
      } else if (!hasOwnProp.call(itemsSeen, item.group)) {
        // replace '/' in url to match ID in some section
        itemsNav += '<li id="' + item.group.replace('/', '_') + '-nav" class="nav-contains"><p>' + linktoFn('', item.group) + '</p>';
        if (steps.length) {
          itemsNav += "<ul class='steps'>";

          steps.forEach(function (step) {
            itemsNav += '<li data-type="method" id="' + item.group.replace('/', '_') + '-' + step.name + '-nav">';
            itemsNav += linkto(step.longname, step.name);
            itemsNav += '</li>';
          });
...
function buildNav(members) {
  ...
  nav += buildStepNav(find({kind: 'step', memberof: {isUndefined: true}}), 'Steps', {}, linkto);
